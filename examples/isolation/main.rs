//! Isolation suite entry point — parses the CLI, runs the selected scenarios
//! for the selected backend(s), and writes raw + summary CSV.
//!
//! # Usage
//!
//! ```text
//! cargo run --release --features bench --example isolation -- [OPTIONS]
//! ```
//!
//! # Options
//!
...
//! | `--idle-gap MS` | `150` | Cooldown between trials (mitigates thermal taint). |
//! | `--out PATH` | `results/isolation.raw.csv` | Raw CSV output path. |
//! | `--summary PATH` | `results/isolation.summary.csv` | Summary CSV output path. |
//! | `--scenarios REGEX` | (all) | Run only scenarios whose name matches. |
...
fn parse_cli() -> Result<Cli, String> {
    let mut backends: Vec<Backend> = vec![Backend::Spmc]; // default: spmc only
    let mut trials = 5usize;
    let mut duration = 1.0f64;
    let mut warmup = 300u64;
    let mut idle_gap = 150u64;
    let mut out = PathBuf::from("results/isolation.raw.csv");
    let mut summary = PathBuf::from("results/isolation.summary.csv");
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--no-pin" => {
                // Pinning is opt-in (bench-pin feature, deferred). No-op here;
                // accepted for forward compatibility with run_all.sh.
            }
...
/// Simple substring match for `--scenarios` (a regex dep would be overkill;
/// a substring is enough for `rate`, `cap`, `n_`, `payload`, etc.).
...
fn main() -> ExitCode {
    let cli = match parse_cli() {
        ...
        Err(e) => {
            ...
            eprintln!("usage: isolation [--backend spmc|sync|both] [--trials N] [--duration SECS] [--warmup MS] [--idle-gap MS] [--out PATH] [--summary PATH] [--scenarios SUBSTR] [--no-pin]");
    ...
    // Remove old outputs so this run is self-contained (not appended to stale data).
    let _ = std::fs::remove_file(&cli.out);
    let _ = std::fs::remove_file(&cli.summary);

    let scenarios = all_scenarios();
    let total = scenarios.iter().filter(|(n, _)| matches(&cli.scenarios, n)).count();
    let mut done = 0;
    ...
    println!("\nisolation: {done} scenario-backend runs complete");
